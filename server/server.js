  ...
  const isValidImageUrl = (url) => {
    if (!url) return true; // Allow empty image field
    try {
      new URL(url);
      return true;
    } catch {
  ...
  app.post("  , async (req, res) => {
    ...
    try {
      ...
      console.log("Using Sarvam API key:", "AIzaSyA4gSzjq8SLiSqJWQ0li5cG2rnN_Kxeayw" ? "Loaded" : "Not loaded");
      const aiRes = await axios.post(
        "https://api.sarvam.ai/v1/chat/completions",
        {
          model: "sarvam-m",
        ...
        },
        {
          headers: {
            "Content-Type": "application/json",
            "x-api-key": "AIzaSyA4gSzjq8SLiSqJWQ0li5cG2rnN_Kxeayw",
          },
        }
      ).catch((error) => {
        if (error.response?.status === 403) {
          throw new Error("Sarvam AI API access denied. Please check your API key or subscription.");
        }
        throw error;
      });
      ...
      const savedRecipe = new GeneratedRecipe({
        user: user._id,
        recipeGenerated: result,
        leftoverItems: leftovers.split(",").map(item => item.trim()),
        createdAt: new Date(),
      });
      await savedRecipe.save();
